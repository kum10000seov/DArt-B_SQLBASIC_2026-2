...
```
- 그룹화 기준: ANIMAL_TYPE
- WHERE와 HAVING 중 사용한 절: WHERE, 집계함수 연산 전 원본 테이블에서 'Cat'과 'Dog' 행만 먼저 분리해내기 위해 사용
- 처음 틀렸다면 틀린 이유: 고양이를 개보다 먼저 조회하는 조건을 GROUP BY 이전에 작성하여 코드 위치 오류(ORDER BY ANIMAL_TYPE ASC;) 
- 새로 배운 SQL 패턴: 그룹화(GROUP BY) 후에도 문제에서 요구하는 순서를 맞추기 위해 ORDER BY를 추가해야 함(순서 명심)
```

<img width="1262" height="695" alt="image" src="https://github.com/user-attachments/assets/3c9a68eb-0fcc-49e7-b063-92f13b9194fc" />
...
```
1. 문제를 SQL로 옮길 때 가장 어려웠던 부분: 일상 언어로 표현된 문제의 조건(고양이를 개보다 먼저, 가장 최근 등)을 어떤 순서로 코딩해야 하는지 구조화하는 과정
2. WHERE와 HAVING의 차이를 어떻게 이해했는지: 데이터를 걸러내는 타이밍(순서), WHERE는 그룹으로 묶기 전 원본 테이블 차원에서 거르는 것, HAVING은 GROUP BY로 그룹화와 집계 연산을 마친 결과값에서 거르는 단계
3. 다음 주에 더 연습하고 싶은 문제 유형: HAVING과 WHERE을 동시에 활용할 수 있는 문제, 일상 언어로 된 표현을 SQL 코드로 구현하는 데 직관적이지 않아 어려운 문제
```
